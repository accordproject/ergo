...
describe('#draft', function () {
    ...
    it('should draft text for a late delivery and penalty contract with else (wrap variable)', async function () {
        const grammarPath = Path.join(TESTS_DIR, 'latedeliveryandpenaltyelse', 'text/grammar.tem.md');
        const ergoPath = Path.join(TESTS_DIR, 'latedeliveryandpenaltyelse', 'logic/logic.ergo');
        const ctoPath = Path.join(TESTS_DIR, 'latedeliveryandpenaltyelse', 'model/model.cto');
        const contractPath = { file: Path.join(TESTS_DIR, 'latedeliveryandpenaltyelse', 'data.json') };
        const result = await Commands.draft(null, [ergoPath, grammarPath, ctoPath], contractPath, '1970-01-01T00:00:00Z', { wrapVariables: true });
    ...
    it('should draft text for a late delivery and penalty contract without force majeure with else (wrap variable)', async function () {
        ...
        const ergoPath = Path.join(TESTS_DIR, 'latedeliveryandpenaltyelse', 'logic/logic.ergo');
        const ctoPath = Path.join(TESTS_DIR, 'latedeliveryandpenaltyelse', 'model/model.cto');
        const contractPath = { file: Path.join(TESTS_DIR, 'latedeliveryandpenaltyelse', 'data-noforcemajeure.json') };
        const result = await Commands.draft(null, [ergoPath, grammarPath, ctoPath], contractPath, '1970-01-01T00:00:00Z', { wrapVariables: true });
