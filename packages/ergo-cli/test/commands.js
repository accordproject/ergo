...
describe('#triggerhello', function () {
    ...
    it('should throw when executing a request to a smart Ergo contract with an illegal model', async function () {
        const ergoPath = Path.join(EXAMPLES_DIR, 'helloworldErr', 'logic/logic.ergo');
        const ctoPath = Path.join(EXAMPLES_DIR, 'helloworldErr', 'model/modelErr.cto');
...
describe('#invokehelloworld', function () {
    ...
    it('should throw when invoking a clause in a smart Ergo contract with an illegal model', async function () {
        ...
        const statePath = { file: Path.join(EXAMPLES_DIR, 'helloworld', 'state.json') };
        return Commands.invoke(null, [ergoPath, ctoPath], 'helloworld', contractPath, statePath, '1970-01-01T00:00:00Z', paramsPath).should.be.rejectedWith('Expected "namespace", comment, end of line, or whitespace but "E" found. File ../../examples/helloworldErr/model/modelErr.cto line 15 column 1');
    });
    ...
    it('should throw when smart Ergo clause without a cto', async function () {
        ...
        const paramsPath = { file: Path.join(EXAMPLES_DIR, 'helloworld3', 'params.json') };
        const statePath = { file: Path.join(EXAMPLES_DIR, 'helloworld3', 'state.json') };
        return Commands.invoke(null, [ergoPath], 'helloworld', contractPath, statePath, '1970-01-01T00:00:00Z', paramsPath).should.be.rejectedWith('Compilation error (at file ../../examples/helloworld3/logic/logic3.ergo line 17 col 25). Cannot find type with name \'TemplateModel\'\ncontract HelloWorld over TemplateModel {\n                         ^^^^^^^^^^^^^  ');
    });
    it('should fail when Ergo logic is missing', async function () {
        const ctoPath = Path.join(EXAMPLES_DIR, 'helloworld', 'model/model.cto');
        const contractPath = { file: Path.join(EXAMPLES_DIR, 'helloworld', 'data.json') };
...
describe('#draft', function () {
    ...
    it('should draft text for a late delivery and penalty contract without force majeure (wrap variable)', async function () {
        ...
        const result = await Commands.draft(null, [ergoPath, grammarPath, ctoPath], contractPath, '1970-01-01T00:00:00Z', { wrapVariables: true });
        result.response.should.equal('Late Delivery and Penalty. In case of delayed delivery<if id="forceMajeure" value=""/> the Seller shall pay to the Buyer for every <variable id="penaltyDuration" value="2%20days"/> of delay penalty amounting to <variable id="penaltyPercentage" value="10.5"/>% of the total value of the Equipment whose delivery has been delayed. Any fractional part of a <variable id="fractionalPart" value="days"/> is to be considered a full <variable id="fractionalPart" value="days"/>. The total amount of penalty shall not however, exceed <variable id="capPercentage" value="55.0"/>% of the total value of the Equipment involved in late delivery. If the delay is more than <variable id="termination" value="15%20days"/>, the Buyer is entitled to terminate this Contract.');
    });
...
describe('#initinstallmentsale', function () {
    it('should invoke init', async function () {
        ...
        const ctoPath = Path.join(EXAMPLES_DIR, 'installment-sale', 'model/model.cto');
        const contractPath = { file: Path.join(EXAMPLES_DIR, 'installment-sale', 'data.json') };
        const paramsPath = { file: Path.join(EXAMPLES_DIR, 'installment-sale', 'params.json') };
    ...
    it('should throw when initializing a smart Ergo contract with an illegal model', async function () {
        const ergoPath = Path.join(EXAMPLES_DIR, 'installment-sale', 'logic/logic.ergo');
        const ctoPath = Path.join(EXAMPLES_DIR, 'installment-sale', 'model/modelErr.cto');
        const contractPath = { file: Path.join(EXAMPLES_DIR, 'installment-sale', 'data.json') };
        const paramsPath = { file: Path.join(EXAMPLES_DIR, 'installment-sale', 'params.json') };
        return Commands.initialize(null, [ergoPath, ctoPath], contractPath, '1970-01-01T00:00:00Z', paramsPath).should.be.rejectedWith('Expected "namespace", comment, end of line, or whitespace but "E" found. File ../../examples/installment-sale/model/modelErr.cto line 15 column 1');
    });
    it('should throw when initializing a smart Ergo clause without a cto', async function () {
        const ergoPath = Path.join(EXAMPLES_DIR, 'installment-sale', 'logic/logic.ergo');
        const contractPath = { file: Path.join(EXAMPLES_DIR, 'installment-sale', 'data.json') };
        const paramsPath = { file: Path.join(EXAMPLES_DIR, 'installment-sale', 'params.json') };
        return Commands.initialize(null, [ergoPath], contractPath, '1970-01-01T00:00:00Z', paramsPath).should.be.rejectedWith('Compilation error (at file ../../examples/installment-sale/logic/logic.ergo line 19 col 30). Cannot find type with name \'TemplateModel\'\ncontract InstallmentSale over TemplateModel state InstallmentSaleState {\n                              ^^^^^^^^^^^^^                             ');
    });
    it('should fail when Ergo logic is missing', async function () {
        const ctoPath = Path.join(EXAMPLES_DIR, 'installment-sale', 'model/model.cto');
        const contractPath = { file: Path.join(EXAMPLES_DIR, 'installment-sale', 'data.json') };
