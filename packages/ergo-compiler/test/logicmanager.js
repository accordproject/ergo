...
describe('LogicManager', () => {
    describe('#constructors-accessors', () => {
        ...
        it('should load a logic file to the script manager', () => {
            ...
            logicManager.getScriptManager().getCompiledScript().getContents().length.should.equal(46905);
            logicManager.compileLogicSync(false);
            logicManager.getScriptManager().getCompiledScript().getContents().length.should.equal(46905);
        });
        ...
        it('should set the compilation target to ES6 and recompile the logic', () => {
            ...
            logicManager.getContractName().should.equal(ErgoCompiler.contractCallName(contractName));
            logicManager.getInvokeCall('helloworld').length.should.equal(213);
            logicManager.getDispatchCall().length.should.equal(197);
            logicManager.getScriptManager().getCompiledScript().getContents().length.should.equal(45284);
        });
    ...
    describe('#loader-dir', () => {
        it('should load a directory with no grammar', async function () {
            ...
            logicManager.getScriptManager().getCompiledScript().getContents().length.should.equal(132950);
        });
        ...
        it('should load a directory with grammar', async function () {
            ...
            modelManager.getModels()[0].content.length.should.equal(1330);
            logicManager.getScriptManager().getCompiledScript().getContents().length.should.equal(70630);
        });
    });
    ...
    describe('#loader-files', () => {
        ...
        it('should load a Zip with grammar', async function () {
            const files = [
                ...
                Path.join(TESTS_DIR,'helloworldcontract/text/grammar.tem.md'),
            ];
            const logicManager = await ErgoLoader.fromFiles(files);
            const modelManager = logicManager.getModelManager();
            modelManager.getModels().map(x => x.name).should.deep.equal([
