...
describe('ergo-compiler', () => {
    ...
    describe('#parsefail', function () {
        it('should fail when Ergo does not parse', async function () {
            const ergoFile = Path.resolve('test', 'examples/bad-logic', 'logic/logic.ergo');
            const ergoContent = Fs.readFileSync(ergoFile, 'utf8');
            const ctoFile = Path.resolve('test', 'examples/bad-logic', 'model/model.cto');
    ...
    describe('#parseerrormessage', function () {
        it('should format parse error', async function () {
            const result = await ErgoCompiler.ergoErrorToString({ 'kind' : 'ParseError', 'message' : 'Parse error', 'fullMessage' : 'Parse error (at file ./test/examples/bad-logic/logic/logic.ergo line 17 col 20). \ncontract HelloWorld ovr TemplateModel {\n                    ^^^                ', 'locstart' : { 'line' : 16, 'column' : 25 }, 'locend' : { 'line' : 16, 'column' : 26 } });
    ...
    describe('#verboseparseerrormessage', function () {
        it('should format fullMessage parse error', async function () {
            const result = await ErgoCompiler.ergoVerboseErrorToString({ 'kind' : 'ParseError', 'message' : 'Parse error', 'fullMessage' : 'Parse error (at file ./test/examples/bad-logic/logic/logic.ergo line 17 col 20). \ncontract HelloWorld ovr TemplateModel {\n                    ^^^                ', 'locstart' : { 'line' : 16, 'column' : 25 }, 'locend' : { 'line' : 16, 'column' : 26 } });
            result.should.deep.equal('Parse error (at file ./test/examples/bad-logic/logic/logic.ergo line 17 col 20). \ncontract HelloWorld ovr TemplateModel {\n                    ^^^                ');
        });
    });
    ...
    describe('#compile', function () {
        it('should compile a smart Ergo contract to JavaScript (ES5)', async function () {
            const ergoFile = Path.resolve('test', 'examples/helloworld', 'logic/logic.ergo');
            const ergoContent = Fs.readFileSync(ergoFile, 'utf8');
    ...
    describe('#parsecto', function () {
        it('should parse CTO', async function () {
            const ctoText = Fs.readFileSync(Path.resolve('test', 'examples/helloworld', 'model/model.cto'), 'utf8');
