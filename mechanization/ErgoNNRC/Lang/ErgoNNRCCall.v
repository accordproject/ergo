...
Section Patch.
  Definition nnrc_lambda_type_of_lambda_type (prov:provenance) (t:option unit) : nnrc_type :=
    ErgoTypeAny prov.

  Definition nnrc_lambda_params_of_lambda_params
             (prov:provenance)
             (params:list (string * option unit)) : list (string * nnrc_type) :=
    List.map (fun xy => (fst xy, nnrc_lambda_type_of_lambda_type prov (snd xy))) params.
    
  Definition nnrc_expr_lambda_of_backend_closure
             (cl:ErgoEnhancedBackend.ergo_backend_closure) : nnrc_expr_lambda :=
    let prov := dummy_provenance in (* XXX TO DECIDE *)
    mkLambdaN
      (nnrc_lambda_params_of_lambda_params prov cl.(Closure.closure_params))
      (nnrc_lambda_type_of_lambda_type prov cl.(Closure.closure_output))
...
Section ErgoNNRCCall.
  ...
  Definition lookup_lambda (prov:provenance) (t:lookup_table) (fname:string) : eresult nnrc_expr_lambda :=
    match t fname with
    | None => function_not_found_error prov fname
    | Some cl => esuccess cl
    end.

  Definition lookup_call (prov:provenance) (t:lookup_table) (fname:string) (el:list nnrc_expr) : eresult nnrc_expr :=
    let rlambda := lookup_lambda prov t fname in
    eolift (fun cl =>
