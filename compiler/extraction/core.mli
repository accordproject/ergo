...
| Cejbigint of int
| Cejbool of bool
| Cejstring of char list
| Cejforeign of foreign_ejson_model
...
| Enhanced_binary_monetary_amount_op of monetary_amount_binary_op
...
| Enhanced_ejson_monetary_amount of ejson_monetary_amount_runtime_op
...
val enhanced_ejson_monetary_amount_runtime_op_interp :
  ejson_monetary_amount_runtime_op -> ejson list -> ejson option
...
type imp_ejson_constant = cejson
...
type imp_ejson_expr =
  (imp_ejson_constant, imp_ejson_op, imp_ejson_runtime_op) imp_expr

type imp_ejson_stmt =
  (imp_ejson_constant, imp_ejson_op, imp_ejson_runtime_op) imp_stmt

module Coq__5 : sig
 type imp_ejson_function =
   (imp_ejson_constant, imp_ejson_op, imp_ejson_runtime_op) imp_function
end
include module type of struct include Coq__5 end

type imp_ejson = (imp_ejson_constant, imp_ejson_op, imp_ejson_runtime_op) imp

val mk_imp_ejson_expr_error :
  foreign_ejson -> foreign_ejson_runtime -> char list -> imp_ejson_expr

val mk_imp_ejson_op :
  foreign_ejson -> foreign_ejson_runtime -> imp_ejson_op ->
  (imp_ejson_constant, imp_ejson_op, imp_ejson_runtime_op) imp_expr list ->
  imp_ejson_expr
...
val mk_left :
  foreign_ejson -> foreign_ejson_runtime -> (imp_ejson_constant,
  imp_ejson_op, imp_ejson_runtime_op) imp_expr -> imp_ejson_expr
...
val mk_object :
  foreign_ejson -> foreign_ejson_runtime -> (char list * imp_ejson_expr) list
  -> imp_ejson_expr
...
val ejson_to_expr :
  foreign_ejson -> foreign_ejson_runtime -> ejson -> imp_ejson_expr
...
val mk_object0 : char list list -> expr list -> expr
...
val mk_integer_plus_one :
  foreign_ejson -> foreign_ejson_runtime -> expr -> expr

val mk_integer_le :
  foreign_ejson -> foreign_ejson_runtime -> expr -> expr -> expr

val imp_ejson_op_to_js_ast : imp_ejson_op -> expr list -> expr

val cejson_to_js_ast :
  foreign_ejson -> foreign_ejson_to_ajavascript -> cejson -> expr
...
val decl_to_js_ast :
  foreign_ejson -> foreign_ejson_to_ajavascript -> foreign_ejson_runtime ->
  (var1 * (imp_ejson_constant, imp_ejson_op, imp_ejson_runtime_op) imp_expr
...
val imp_ejson_function_to_funcelement :
  foreign_ejson -> foreign_ejson_to_ajavascript -> foreign_ejson_runtime ->
  char list -> (imp_ejson_constant, imp_ejson_op, imp_ejson_runtime_op)
...
val make_as_double_fun :
  brand_model -> char list -> provenance -> QcertType.qtype ->
  (provenance * QcertType.qtype, provenance, absolute_name) ergo_expr ->
  ergoct_expr
