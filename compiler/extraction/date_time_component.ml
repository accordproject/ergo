...
(** Duration *)
type duration = Calendar.Time.Period.t
let duration_amount (x:duration) : int = Calendar.Time.Period.to_seconds x
let duration_eq (d1:duration) (d2:duration) : bool = (duration_amount d1) = (duration_amount d2)
...
let duration_from_seconds (x:int) : duration = Calendar.Time.Period.make 0 0 x
let duration_from_minutes (x:int) : duration = Calendar.Time.Period.make 0 x 0
let duration_from_hours (x:int) : duration = Calendar.Time.Period.make x 0 0
let duration_from_days (x:int) : duration = Calendar.Time.Period.make (24 * x) 0 0
let duration_from_weeks (x:int) : duration = Calendar.Time.Period.make (24 * 7 * x) 0 0
...
let period_from_days (x:int) : period = Calendar.Period.make 0 0 x 0 0 0
let period_from_weeks (x:int) : period = Calendar.Period.make 0 0 (x * 7) 0 0 0
let period_from_months (x:int) : period = Calendar.Period.make 0 x 0 0 0 0
let period_from_quarters (x:int) : period = Calendar.Period.make 0 (x * 3) 0 0 0 0
let period_from_years (x:int) : period = Calendar.Period.make x 0 0 0 0 0
...
(** Components *)
let get_seconds (x:dateTime) : int = Calendar.Time.second (Calendar.to_time x)
let get_minutes (x:dateTime) : int = Calendar.Time.minute (Calendar.to_time x)
let get_hours (x:dateTime) : int = Calendar.Time.hour (Calendar.to_time x)
let get_days (x:dateTime) : int = Calendar.day_of_month x
let get_weeks (x:dateTime) : int = Calendar.week x
let get_months (x:dateTime) : int = Date.int_of_month (Calendar.month x)
let get_quarters (x:dateTime) : int = ((get_months x) / 3) + 1
let get_years (x:dateTime) : int = Calendar.year x
...
(** Arithmetics *)
let diff (x1:dateTime) (x2:dateTime) : duration = Calendar.Period.to_time (Calendar.sub x1 x2)
let diff_days (x1:dateTime) (x2:dateTime) : float =
  let dur = diff x1 x2 in
  (Time.Period.to_hours dur) /. 24.0
let diff_seconds (x1:dateTime) (x2:dateTime) : float =
  let dur = diff x1 x2 in
  Time.Second.to_float (Time.Period.to_seconds dur)

let add (x1:dateTime) (d1:duration) : dateTime = Calendar.add x1 (Calendar.Period.from_time d1)
let subtract (x1:dateTime) (d1:duration) : dateTime = Calendar.rem x1 (Calendar.Period.from_time d1)
...
let duration_seconds (x:int) : duration = Calendar.Time.Period.second x
let duration_minutes (x:int) : duration = Calendar.Time.Period.minute x
let duration_hours (x:int) : duration = Calendar.Time.Period.hour x
let duration_days (x:int) : duration = Calendar.Time.Period.hour (24 * x)
let duration_weeks (x:int) : duration = Calendar.Time.Period.hour (7 * 24 * x)

let period_days (x:int) : period = Calendar.Period.day x
let period_weeks (x:int) : period = Calendar.Period.week x
let period_months (x:int) : period = Calendar.Period.month x
let period_quarters (x:int) : period = Calendar.Period.month (x * 3)
let period_years (x:int) : period = Calendar.Period.year x
