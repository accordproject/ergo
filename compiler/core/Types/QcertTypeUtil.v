...
Section QcertTypeUtil.
  Context {m : brand_model}.

  Import QcertType.

  Definition qcert_type_join_safe prov (t1 t2:qcert_type) : eresult qcert_type :=
    let jt := qcert_type_join t1 t2 in
    if qcert_type_subtype_dec ttop jt
    then efailure (ETypeError prov ("Join between types is TOP.")%string)
    else esuccess jt nil.

  Definition qcert_type_meet_safe prov (t1 t2:qcert_type) : eresult qcert_type :=
    let jt := qcert_type_meet t1 t2 in
    if qcert_type_subtype_dec jt tbottom
    then efailure (ETypeError prov ("Meet between types is BOTTOM.")%string)
    else esuccess jt nil.

  Program Definition empty_rec_type : qcert_type := Rec Closed nil _.

  Definition ergo_format_unop_error nsctxt (op : unary_op) (arg : qcert_type) : string :=
    let fmt_easy :=
        fun name expected actual =>
          ("Operator `" ++ name ++ "' expected an operand of type `" ++
                        (qcert_type_to_string nsctxt expected) ++
    ...
    | OpDot name => "The field `" ++ name ++ "' does not exist in type `" ++ (qcert_type_to_string nsctxt arg) ++ "'"
    | OpIdentity | OpRec _ | OpBag
    | OpLeft | OpRight | OpBrand _ | OpUnbrand | OpCast _ =>
      "This operator received an unexpected argument of type `" ++ (qcert_type_to_string nsctxt arg) ++ "'"
    | OpRecRemove _ | OpRecProject _
    | OpSingleton | OpFlatten | OpDistinct | OpOrderBy _
    | OpCount | OpToString | OpToText | OpLength | OpSubstring _ _ | OpLike _
    ...
    | OpForeignUnary _ =>
      "This function received an unexpected argument of type `" ++ (qcert_type_to_string nsctxt arg) ++ "'"
    end.

  Definition ergo_format_binop_error nsctxt (op : binary_op) (arg1 : qcert_type) (arg2 : qcert_type) : string :=
    let fmt_easy :=
        fun name e1 e2 =>
          ("Operator `" ++ name ++ "' expected operands of type `" ++
                        (qcert_type_to_string nsctxt e1) ++ "' and `" ++
                        (qcert_type_to_string nsctxt e2) ++
                        "' but received operands of type `" ++
                        (qcert_type_to_string nsctxt arg1) ++ "' and `" ++
                        (qcert_type_to_string nsctxt arg2) ++ "'.")%string
    in
    match op with
    ...
    | OpEqual | OpStringConcat | OpStringJoin
      => "This operator received unexpected arguments of type `" ++ (qcert_type_to_string nsctxt arg1) ++ "' " ++ " and `" ++ (qcert_type_to_string nsctxt arg2) ++ "'."
  ...
  Definition ergo_format_unary_operator_dispatch_error nsctxt (op : ergo_unary_operator)
     (arg : qcert_type) : string :=
    "This operator received an unexpected argument of type `" ++ (qcert_type_to_string nsctxt arg) ++ "'.".

  Definition ergo_format_binary_operator_dispatch_error nsctxt (op : ergo_binary_operator)
     (arg1 : qcert_type) (arg2 : qcert_type) : string :=
    "This operator received unexpected arguments of type `" ++ (qcert_type_to_string nsctxt arg1) ++ "' " ++ " and `" ++ (qcert_type_to_string nsctxt arg2) ++ "'.".

  Definition ergo_format_new_error nsctxt (name:string) (actual:qcert_type) : string :=
    let concept_name := qcert_type_to_string nsctxt (Brand (name::nil)) in
    ...
    | Some (nil, nil) =>
      ...
      | (expected_name, expected_type, actual_type) :: _ =>
        "Field `" ++ expected_name
                  ++ "' has type `" ++ (qcert_type_to_string nsctxt actual_type)
                  ++ "' but should have type `" ++ (qcert_type_to_string nsctxt expected_type) ++ "'"
      end
  ...
  Definition ergo_format_clause_return_fallback_error
             nsctxt
             (name:string)
             (actual expected:qcert_type) : string :=
    let actual_s := qcert_type_to_string nsctxt actual in
    let expected_s := qcert_type_to_string nsctxt expected in
  ...
  Definition ergo_format_clause_return_normal_error
             nsctxt
             (name:string)
             (actual expected:qcert_type)
             (actual_quad expected_quad:qcert_type * qcert_type * qcert_type * qcert_type)
    : string :=
    ...
    if qcert_type_subtype_dec actual_resp expected_resp
    then
      if qcert_type_subtype_dec actual_emit expected_emit
      then
        if qcert_type_subtype_dec actual_state expected_state
        then
          if qcert_type_subtype_dec actual_error expected_error
          then
  ...
  Definition ergo_format_clause_return_error nsctxt (name:string) (actual expected:qcert_type) : string :=
  ...
  Definition ergo_format_function_return_error nsctxt (name:string) (actual expected:qcert_type) : string :=
    let actual_s := qcert_type_to_string nsctxt actual in
    let expected_s := qcert_type_to_string nsctxt expected in
    "Function " ++ name ++ " should return `" ++ expected_s ++ "' but actually returns `" ++ actual_s ++ "'".

  Definition make_unary_operator_criteria op nsctxt prov t : eresult qcert_type :=
    match qcert_type_infer_unary_op op t with
    | Some (r, _) => esuccess r nil
    | None => efailure (ETypeError prov (ergo_format_unop_error nsctxt op t))
    end.

  Definition make_binary_operator_criteria op nsctxt prov t1 t2 : eresult qcert_type :=
    match qcert_type_infer_binary_op op t1 t2 with
