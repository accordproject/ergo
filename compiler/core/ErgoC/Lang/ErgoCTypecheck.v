...
Section ErgoCTypecheck.
  ...
  Fixpoint ergoc_expr_typecheck nsctxt (ctxt : type_context) (expr : ergoc_expr) : eresult ergoct_expr :=
    match expr with
    ...
    | ESome prov e =>
      elift (fun eT => ESome (prov,toption (exprct_type_annot eT)) eT) (ergoc_expr_typecheck nsctxt ctxt e)
    | EArray prov es =>
      elift (fun eT => EArray (prov,tcoll (snd eT)) (fst eT))
            (fold_right
               (fun (new:ergoc_expr) (eT:eresult (list ergoct_expr * qcert_type)) =>
                  eolift
                    (fun (eT':list ergoct_expr * qcert_type) =>
                       elift
                         (fun (new':ergoct_expr) =>
                            (new'::(fst eT'), qcert_type_join (exprct_type_annot new') (snd eT')))
                         (ergoc_expr_typecheck nsctxt ctxt new))
                    eT)
    ...
    | EBinaryOperator prov eop e1 e2 =>
      eolift2 (binary_dispatch nsctxt prov eop)
              (ergoc_expr_typecheck nsctxt ctxt e1)
              (ergoc_expr_typecheck nsctxt ctxt e2)
    ...
    | EIf prov c t f =>
      eolift (fun cT' =>
                if qcert_type_subtype_dec (exprct_type_annot cT') tbool then
                  elift2 (fun eT1 eT2 =>
                            let t1 := exprct_type_annot eT1 in
                            let t2 := exprct_type_annot eT2 in
                            EIf (prov,qcert_type_join t1 t2) cT' eT1 eT2)
                         (ergoc_expr_typecheck nsctxt ctxt t)
                         (ergoc_expr_typecheck nsctxt ctxt f)
                else efailure (ETypeError (expr_annot c) "'if' condition not boolean."%string))
             (ergoc_expr_typecheck nsctxt ctxt c)
    | ELet prov n None v e =>
      (eolift (fun evT =>
                 ...
                 elift (fun eT => ELet (prov, exprct_type_annot eT) n None evT eT)
                       (ergoc_expr_typecheck nsctxt ctxt' e))
              (ergoc_expr_typecheck nsctxt ctxt v))
    | ELet prov n (Some t) v e  =>
          ...
          | ProvFunc _ fname =>
            ETypeError prov
                       ("Function `" ++ fname
                                     ...
                                     ++ n
                                     ++ "' to be of type `"
                                     ++ (qcert_type_to_string nsctxt t')
                                     ++ "' but was given argument of type `"
                                     ++ (qcert_type_to_string nsctxt vt)
                                     ++ "'." )
          | _ => ETypeError prov
      ...
      (eolift
         (fun evT =>
            let vt := exprct_type_annot evT in
            let t' := (ergo_type_to_qcert_type t) in
    ...
    | ERecord prov rs =>
      elift (fun eT => ERecord (prov,(snd eT)) (fst eT))
            (fold_right
               (fun (next:string * ergoc_expr) (sofar:eresult (list (string * ergoct_expr) * qcert_type)) =>
                  eolift2
                    (fun (sofar':list (string * ergoct_expr) * qcert_type)
                         (next':(string * ergoct_expr) * qcert_type) =>
                       elift (fun T' => (fst next'::fst sofar',compose fst fst T'))
                             (eresult_of_option
                                (qcert_type_infer_binary_op OpRecConcat (snd next') (snd sofar'))
                    ...
                    (eolift (fun efT =>
                               ...
                               elift (fun T => ((fst next,efT), fst T))
                                     (eresult_of_option
                                        (qcert_type_infer_unary_op (OpRec (fst next)) ft)
        ...
        (fun eT0 dT =>
          ...
          elift
            ...
            (fold_right
               (fun pe sofarT =>
                  ...
                  | (CaseLet prov name None, res) =>
                    elift2 (fun sofarT eT =>
                           ...
                           sofarT
                           (ergoc_expr_typecheck nsctxt (type_context_update_local_env ctxt name t0) res)
                  ...
                  | (CaseLetOption prov name None, res) =>
                    ...
                    | Some (st, ft) =>
                      elift2 (fun sofarT eT =>
                             ...
                             sofarT
                             (ergoc_expr_typecheck nsctxt (type_context_update_local_env ctxt name st) res)
                    end
               ...
               (esuccess (nil, dt) nil)
               pes))
        (ergoc_expr_typecheck nsctxt ctxt term)
        (ergoc_expr_typecheck nsctxt ctxt default)
           ...
           eolift
             (fun typ =>
                (elift
                   ...
                   (ergoc_expr_typecheck nsctxt (type_context_update_local_env ctxt name typ) fn)))
             (eresult_of_option
                (untcoll arr')
                (ETypeError
                   prov
                   ("foreach expects an array to iterate over, but was given something of type `" ++ (qcert_type_to_string nsctxt arr') ++ "'.")) nil))
        (ergoc_expr_typecheck nsctxt ctxt arr)
    | EForeach prov _ _ _ =>
      ...
      eolift
        (fun outT =>
           ...
           | None => esuccess (mkFuncCT
                               ...
                               dctxt) nil
           | Some eoutt' =>
             let expectedt := ergo_type_to_qcert_type eoutt' in
  ...
  Definition ergoc_clause_typecheck
    ...
    let (name,body) := cl in
    elift (fun fT => ((name,fst fT), snd fT)) (ergoc_function_typecheck nsctxt name dctxt body).

  Definition ergoc_contract_typecheck
             (nsctxt: namespace_ctxt)
             (dctxt : type_context)
    ...
    elift (fun cT =>
          ...
          (elift_context_fold_left
             (ergoc_clause_typecheck nsctxt)
             c.(contractc_clauses)
             dctxt).

  Definition ergoc_decl_typecheck
             (nsctxt: namespace_ctxt)
             (dctxt : type_context)
    ...
    match decl with
    | DCExpr prov expr =>
      let exprT := ergoc_expr_typecheck nsctxt dctxt expr in
    ...
    | DCFunc prov name func =>
      elift (fun fc => (DCTFunc prov name (fst fc),snd fc))
            (ergoc_function_typecheck nsctxt name dctxt func)
  ...
  Definition ergoc_module_typecheck
             (nsctxt: namespace_ctxt)
             (dctxt : type_context)
             (mod : ergoc_module)
