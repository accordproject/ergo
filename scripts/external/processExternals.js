...
/**
 * Translate CTO URL to an OCaml file name
 *
 * @param {string} requestUrl - the URL of the CTO file
 * @param {string} the file name
 */
function mapNameOCaml(requestUrl) {
    let fileName = mapNameExternal(requestUrl);
    const reg = /@((\d+.)+\d+)/gi; // Remove option tag from file name
    fileName = fileName.replace(reg, '');
    let fileNameSplit = fileName.split('.');
...
function mapContentOCaml(content) {
    const ctoJson = CTOParser.parse(content);
    return JSON.stringify(ctoJson);
}
...
/**
 * Fetches all external for a set of models dependencies and
 * saves all the OCaml versions of the models to a target directory
 *
 * @param {object} modelManager - the model manager
 * @param {string} output - the output directory
 * @param {boolean} clean - true if deleting the files
 */
function buildOCaml(modelManager, output, clean) {
    buildAux(modelManager, output, clean, 'ocamltarget.hbs', mapNameOCaml, mapContentOCaml, '.ml');
}
...
async function process(dir, workload, clean = false) {
    ...
    if (workload.ocaml) {
        buildOCaml(modelManager, path.join(dir, workload.ocaml), clean);
    }
}
